...
const InfraPpmScheduleDetails = (props: any) => {
  ...
  var AssetDetails = location?.state?.assetTreeDetails || {};
  ...
  const onSubmit = useCallback(
    async (payload: any, e: any) => {
      ...
      try {
        ...
        //  payload.SCHEDULE_INFRA_DAYS = payload.OCCURANCE?.DAY_NUM;
        payload.PARA =
          buttonMode === "CANCEL"
            ? { para1: `PPM Schedule `, para2: "Cancelled" }
        ...
        if (res?.FLAG) {
          ...
          const notifcation: any = {
            ...
            PARA4: moment(options?.ppmSchedular?.SCHEDULE_DATE).format(
              "DD-MM-YYYY"
            ),
        ...
        toast.error(
          error === null ? "Something went wrong" : error?.toString()
        );
      } finally {
        setIsSubmit(false);
      }
  ...
  const getOptions = async () => {
    ...
    await getEquipmentGroup(
      res?.ASSETDETAILS[0]?.ASSETGROUP_ID,
      res?.ASSETDETAILS[0]?.ASSET_ID
    );
  };
                ...
                <Field
                  controller={{
                    ...
                    render: ({ field }: any) => {
                      return (
                        <Select
                          options={INFRS_EQUIPMENT_OPTIONS?.dayNumList}
                          {...register("OCCURANCE", {
                            required: "Please fill the required fields",
                          })}
                          label="How many days the maintenance will last? "
                          optionLabel={"VIEW"}
                          // findKey={key}
            ...
            <div className="mt-2 grid grid-cols-1 gap-x-2 gap-y-2 md:grid-cols-3 lg:grid-cols-3">
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  Code :{" "}
                  {options?.assetDetails?.ASSET_CODE === null
                    ? "NA"
                    : options?.assetDetails?.ASSET_CODE}
                </label>
              ...
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  Group :{" "}
                  {options?.assetDetails?.ASSETGROUP_NAME === null
                    ? "NA"
                    : options?.assetDetails?.ASSETGROUP_NAME}
                </label>
              </div>
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  Make :{" "}
                  {options?.assetDetails?.MAKE_NAME === null
                    ? "NA"
                    : options?.assetDetails?.MAKE_NAME}
                </label>
              </div>
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  Warranty End date :{" "}
                  {helperNullDate(options?.assetDetails?.WARRANTY_END_DATE) ===
                    ""
                    ? "NA"
              ...
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  Name :{" "}
                  {options?.assetDetails?.ASSET_NAME === null
                    ? "NA"
                    : options?.assetDetails?.ASSET_NAME}
                </label>
              </div>
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  Type :{" "}
                  {options?.assetDetails?.ASSETTYPE_NAME === null
                    ? "NA"
                    : options?.assetDetails?.ASSETTYPE_NAME}
                </label>
              </div>
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  Model :{" "}
                  {options?.assetDetails?.MODEL_NAME === null
                    ? "NA"
                    : options?.assetDetails?.MODEL_NAME}
                </label>
              </div>
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  Location :{" "}
                  {options?.assetDetails?.LOCATION_NAME === null
                    ? "NA"
                    : options?.assetDetails?.LOCATION_NAME}
                </label>
              </div>
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  Under AMC :
                  {options?.assetDetails?.USERAMC === true ? " Yes" : " No"}
                </label>
              </div>
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  AMC Vendor :{" "}
                  {options?.assetDetails?.VENDOR_NAME === null
                    ? "NA"
                    : options?.assetDetails?.VENDOR_NAME}
                </label>
              </div>
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  AMC Expiry :{" "}
                  {helperNullDate(options?.assetDetails?.AMC_EXPIRY_DATE) === ""
                    ? "NA"
          ...
          <Card className="mt-2">
            ...
            <div className="mt-2 ">
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  Equipment Name :{" "}
                  {AssetDetails[0]?.ASSET_NAME === null
                    ? "NA"
                    : AssetDetails[0]?.ASSET_NAME}
                </label>
              </div>
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  Equipment Group :{" "}
                  {AssetDetails[0]?.ASSETGROUP_NAME === null
                    ? "NA"
                    : AssetDetails[0]?.ASSETGROUP_NAME}
                </label>
              </div>
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  Equipment Type :{" "}
                  {AssetDetails[0]?.ASSETTYPE_NAME === null
                    ? "NA"
                    : AssetDetails[0]?.ASSETTYPE_NAME}
                </label>
              </div>
              <div className="flex">
                <label className="Text_Secondary Input_Label mr-2">
                  Location :{" "}
                  {AssetDetails[0]?.LOCATION_DESCRIPTION === null
                    ? "NA"
                    : AssetDetails[0]?.LOCATION_DESCRIPTION}
                </label>
              </div>
            </div>
          </Card>
...
export default InfraPpmScheduleDetails;
